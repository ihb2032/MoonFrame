...
- **Express** — build composable column expressions
  ...
  closure row by row.
- **Defer & optimize** — `lazy_frame(df)`, or `scan_csv("sales.csv")` for a
  lazy CSV source, builds a query plan you can `explain()`; `collect()` runs
  it through a predicate- and projection-pushdown optimizer, bitwise-equal to
  the eager pipeline. Projection pushdown narrows a `scan_csv` to the columns
  the pipeline reads, so only those are parsed.
