...
## v0.6.0 — the additive era begins

The first release in the additive era that v0.5 promised: every v0.5.3 symbol
and signature is unchanged, so no code changes are required to upgrade
([`migration.md`](migration.md)). What is new is a handful of API-consistency
aliases and a facade completeness fix — all purely additive — over a round of
