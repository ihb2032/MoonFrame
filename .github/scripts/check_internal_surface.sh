...
"
  ...
  for sym in $syms; do
    if ! printf '%s\n' "$written" | grep -qx -- "$sym"; then
      ...
      # rule can find its callers, so it is listed and left out of the audit
      # rather than checked.
      generated_n=$((generated_n + 1))
      generated_list="$generated_list$pkg/$sym
"
      continue
    fi
    checked=$((checked + 1))
...
# The `pub impl` / `pub extend` lines join the derived methods already listed
# per symbol above: real surface, reachable only through `==` or a trait bound,
# so no name-based rule can find their callers.
#
# They are pinned rather than merely counted. A number going up says something
# grew; it does not say what, and the review that found thirteen of these
# published only because a test wanted `==` had to read every interface by hand
# to find out. The snapshot makes the next one a diff: a symbol appearing here
# is a question — does production need this, or is a test reaching for the
# convenience of an operator? An equality a package's own tests want belongs in
# a `_wbtest.mbt`, which compiles inside the package and publishes nothing.
for mbti in $(git ls-files 'internal/*/pkg.generated.mbti'); do
  ...
  lines=$(grep '^pub impl \|^pub extend ' "$mbti" || true)
  [ -z "$lines" ] || generated_list="$generated_list$(printf '%s\n' "$lines" |
    sed "s|^|$pkg/|")
"
done

generated_snapshot=".github/scripts/internal_generated.snapshot"
generated_now=$(printf '%s' "$generated_list" | grep . | LC_ALL=C sort -u)
if [ "$write" -eq 1 ]; then
  printf '%s\n' "$generated_now" >"$generated_snapshot"
  printf 'internal surface: wrote %s (%s entries)\n' "$generated_snapshot" \
    "$(printf '%s' "$generated_now" | grep -c . || true)"
fi
if [ -f "$generated_snapshot" ] && [ "$write" -eq 0 ]; then
  if ! gen_diff=$(printf '%s\n' "$generated_now" |
    diff -u "$generated_snapshot" - 2>&1); then
    printf 'internal surface: the derived / impl surface changed:\n'
    printf '%s\n' "$gen_diff" | sed 's/^/  /'
    printf '  These are outside the name-based audit — nothing can prove who\n'
    printf '  calls an `equal` reached through `==`. So the set is pinned: one\n'
    printf '  appearing is a decision to make rather than a number to watch.\n'
    printf '  If a test wanted the operator, the `impl` belongs in a\n'
    printf '  `_wbtest.mbt`, which compiles inside the package and publishes\n'
    printf '  nothing. If production needs it, regenerate:\n'
